...
INSERT INTO {job}_{target}
SELECT zinc_id, smiles, mw, logp, rotb, hbd, hba, q
  FROM {schema}.{part} db
 WHERE EXISTS (SELECT 1 FROM {job}_a_fps
                WHERE tanimoto_sml(fp, db.mfp2) > {x} AND target <> '{target}'
                ORDER BY RANDOM()
                LIMIT 40)
   AND NOT EXISTS (SELECT 1 FROM {job}_a_fps
                    WHERE tanimoto_sml(fp, db.mfp2) > {tc} AND target = '{target}');
  SELECT COUNT(*) FROM {job}_{target};
"""
...
def get_simi(kwargs):
    ...
    for target in kwargs['targets']:
        ...
        print("Time for query {} decoys for {} from {}: {}".format(
            _cursor.fetchone()[0],
            target, kwargs['part'],
            dt.now() - _t))
    _connect.commit()
...
create_simi_table_query = """
CREATE TABLE {job}_{target} (
            zinc_id integer,
            smiles text,
            mw real,
            logp real,
...
"""
for i, target in enumerate(targets):
    cursor.execute(create_simi_table_query.format(job=job_name, target=target))
connect.commit()
...
N = len(part_kwargs)
for decoys in tqdm(pool.imap_unordered(get_simi, part_kwargs), total=N):
    pass
rm_actives = """
    DROP TABLE IF EXISTS {job}_a_fps;
    DROP TABLE IF EXISTS {job}_d_fps;
